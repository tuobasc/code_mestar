...
greedy_tsp_query = """You are an expert in algorithms and data structures.
...
def tsp_query_greedy(dataset_name_list, max_trys=9, model="gpt-4o-mini"):
    file_path_list = []
    ...
    fitness_list = []
    for _ in range(max_trys):
        res, input_tokens, output_tokens = request(greedy_tsp_query, temperature=0.5, model=model)
        code = parser_codes(res)
        print(code)
        try:
